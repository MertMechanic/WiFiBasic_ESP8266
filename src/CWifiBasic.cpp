...
/**
 * @brief Delete WifiSettings & ConfigFile
 */
void CWifiBasic::deleteWifiConfig()
{
    Serial.println("Deleting WIFI SETUP");
    if (this->m_isInAPMode  == false    &&
        this->m_pIP         != nullptr  && 
        this->m_pSubnetmask != nullptr  && 
        this->m_pGateway    != nullptr)
    {
    ...
    String str;
    CFileSystem::getInstance().readFile("/config.json", &str);
